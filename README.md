...
- `docs/` — operating, editorial, research, design, publishing, decision, and technical documentation
...
Website articles currently live in `src/lib/posts.ts`. The current TypeScript-based publishing process is documented in:
...
`src/routeTree.gen.ts` is generated by TanStack Router and is committed because the application build and repository checks depend on it.

- Do not edit it by hand.
- Route-file additions, removals, or renames should regenerate it.
- Ordinary article changes in `src/lib/posts.ts` should not change it.
- Review generated diffs for expected route identities before committing them.

The complete generated-file policy is documented in `docs/08_Generated_Files_Policy.md`.
...
- `docs/07_Performance_Baseline.md`
- `docs/08_Generated_Files_Policy.md`
